...
public class StartupDataLoader implements javax.servlet.ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent event) {
		...
		fullTextSession.index(salesCloser);
		...
		fullTextSession.index(football);
		...
		// NOTE:  The "active" flag for this App is set to "false", so the entity should not be indexed by Lucene.  However, FullTextSession.index() 
		// does NOT respect the EntityIndexingInterceptor used to drive this conditional indexing.  Therefore, you must programatically ensure 
		// that FullTextSession.index() isn't called for objects you don't want indexed.  However, MassIndexer does respect conditional indexing, if 
		// you go that route with manual updates.
		//fullTextSession.index(crystal);
		...
		fullTextSession.index(pencilSharpener);
		...
		fullTextSession.index(staplerTracker);
		...
		fullTextSession.index(frustratedFlamingos);
		...
		fullTextSession.index(domeBrowser);
		...
		fullTextSession.index(athenaRadio);
		...
		fullTextSession.index(mapJourney);
		logger.info("Persisting " + mapJourney.getName());
		
		fullTextSession.getTransaction().commit();

		// This is unnecessarily redundant, since we've already been indexing entities individually with "fullTextSession.index()" along 
		// the way, but the code below also demonstrates how a MassIndexer would be used instead.
		try {
			fullTextSession.createIndexer().startAndWait();
		} catch (InterruptedException e) {
			logger.error("MassIndexer interrupted: " + e.getClass().getName() + ", " + e.getMessage());
		}
